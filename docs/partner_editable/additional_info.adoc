...
```
NOTE: See the Iridium Short Burst Data Service Developers Guide for allowed combinations. For field details, see the following link:#_top_level_mt_keys[Top-level MT keys] table.
...
NOTE: For transceiver message sizes and other specifications, see the Iridium Short Burst Data Service Developers Guide.