...
* Manually, by navigating to Amazon SQS queues in the AWS console.
...
The recommended approach for receiving MO messages is to long-poll the ICCMO.fifo queue. After messages are received, save your database to process the messages. Then delete the messages.
...
```
NOTE: See the Iridium Short Burst Data Service Developers Guide for more information about message specifications.
...
|cdr_reference |Call detail record, also known as auto ID. Unique identifier for a given message in the Iridium gateway database. |10-digit number

|imei |International Mobile Equipment Identity (IMEI). Unique equipment identifier, also known as device ID. |15-digit number
...
```
NOTE: See the Iridium Short Burst Data Service Developers Guide for allowed combinations. For field details, see the link:#_top_level_mt_keys[Top-level MT keys] table below.
...
|assign_mtmsn |When this flag is set, the Generic Security Standard (GSS) API will use the value in the Unique ID field in the message header as the MTMSN for the associated MT message payload. |Boolean (true, false)
|===
...
NOTE: See the Iridium Short Burst Data Service Developers Guide for transceiver message sizes and other specifications.