...
. On the *Specify stack details* page, change the stack name if needed. Review the parameters for the template. Provide values for the parameters that require input. For all other parameters, review the default settings and customize them as necessary. (See link:#_parameter_reference[Parameter reference].) Each deployment takes about {deployment_time} to complete.
